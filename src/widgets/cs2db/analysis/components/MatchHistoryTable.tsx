import { pick } from "../analysisService";
import type { JsonValue } from "../types";

export function MatchHistoryTable({ matches }: { matches: JsonValue[] }) {
  if (matches.length === 0) {
    return <p className="analysis__muted">No recent matches found.</p>;
  }

  return (
    <table className="analysis__match-table">
      <thead>
        <tr>
          <th>Date</th>
          <th>Map</th>
...
function resultClass(result: string | undefined): string {
  ...
  if (normalized.includes("win")) return "analysis__result-win";
  if (normalized.includes("loss") || normalized.includes("lose")) return "analysis__result-loss";
  return "";
}
